...
(function (factory) {
  ...
  console.log("Loaded imageScale.jssssssssssssssssssssssssssss");
      ...
      if (!data) {
        var didLoad = $img[0].complete,
          ...
          loadFunc = function () {
            $this.data(
              "imageScale",
              (data = new ImageScale(that, formattedOpt))
            );
  ...
  var ImageScale = function (element, options) {
    ...
    var $element = (that.$element = $(element)),
      $img = (that.$img =
        element.tagName === "IMG" ? $element : $element.find("img")),
      img = (that.img = $img[0]);
    ...
    var $parent = (that.$parent = options.parent
      ? options.parent
      : $($element.parent()[0]));
    ...
    */
    scale: function (firstTime, opt) {
      ...
      var destWidth = opt.destWidth ? opt.destWidth : $parent.outerWidth(),
        ...
        destInnerHeight = opt.destHeight
          ? opt.destHeight
          : $parent.innerHeight(),
      ...
      var layout = this._innerFrameForSize(
        scale,
        align,
        sourceWidth,
        sourceHeight,
